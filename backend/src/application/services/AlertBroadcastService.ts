...
export class AlertBroadcastService implements IAlertBroadcastService {
  ...
  async broadcast(
    ...
    if (!options?.skipBroadcast) {
      if (options?.feedItem) {
        await this.broadcastFeedToCity(event.city, options.feedItem);
      }

      if (options?.legacyFeedItem) {
        await this.cityRoomService.broadcastToCity(
          event.city,
          SOCKET_EVENTS.PANIC_ALERTS,
          options.legacyFeedItem,
  ...
  private async broadcastFeedToCity(city: string, feedItem: FeedEventItem): Promise<void> {
